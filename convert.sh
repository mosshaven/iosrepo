...
# Auto IPA to DEB Converter with Repository Update
# Usage: ./convert-and-update.sh [input.ipa] [--watch]
...
# Watch mode - monitor input directory for new IPA files
watch_mode() {
    ...
    if command -v inotifywait >/dev/null 2>&1; then
        while true; do
            ...
            # Process all IPA files in input directory
            ...
            # Update repository if we have new packages
            update_repository
        done
    else
...
# Convert all IPA files in input directory
convert_all() {
    ...
    for ipa_file in "$INPUT_DIR"/*.ipa; do
        if [ -f "$ipa_file" ]; then
            if convert_ipa "$ipa_file"; then
                ...
                ((failed_count++))
                # Move failed file to avoid reprocessing
